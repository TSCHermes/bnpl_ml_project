...
# CatBoost might not be installed, we'll skip if not available
try:
...
# Define 20+ models with reasonable default hyperparameters
def get_models():
    models = []
    
    # Linear Models
    models.append(("LogisticRegression", LogisticRegression(max_iter=1000, random_state=42)))
    models.append(("LogisticRegression_L1", LogisticRegression(penalty='l1', solver='saga', max_iter=1000, random_state=42)))
    ...
    # SVM
    models.append(("SVC_Linear", SVC(kernel='linear', probability=True, random_state=42)))
    models.append(("SVC_RBF", SVC(kernel='rbf', probability=True, random_state=42)))
    ...
    # Additional ensemble
    models.append(("BaggingClassifier", BaggingClassifier(random_state=42))) if 'BaggingClassifier' in dir() else None
    
    # Filter out None
    models = [m for m in models if m is not None]
    return models

# Import BaggingClassifier if available
try:
    from sklearn.ensemble import BaggingClassifier
except ImportError:
    BaggingClassifier = None

def evaluate_model(name, model, X_train, X_test, y_train, y_test, config_name):
...
for config_name, preprocess in configs:
    for model_name, model in models:
        ...
        try:
            result = evaluate_model(model_name, model, X_train, X_test, y_train, y_test, config_name)
